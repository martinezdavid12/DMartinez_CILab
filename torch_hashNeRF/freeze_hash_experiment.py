# turning off hashtable test
# after reaching about 30-33 PSNR, try leaving the hash table as is and only optimize the NN
import os
import torch
import torchvision
...
def saveImage(frame_note, psnr_note=39):
    ...
    with torch.inference_mode():
        ...
        plt.axis(False)
        plt.savefig("c_elegans_reconstructions_frozen_hash/hash_nerf_reconstruction_frame_"+str(frame_note)+"_psnr_"+str(psnr_note)+".png", bbox_inches="tight", pad_inches=0.0)
        plt.close()
...
for t in range(0, numFrames):
    ...
    optimizer = torch.optim.Adam(params=model_0.parameters(), lr=lr1, eps=10e-15, weight_decay=10e-6)
    # Training Loop
    start = timer()
    PSNR_thresh = 38
    batchCount = 0
    ...
    for epoch in tqdm(range(0,100)):
        ...
        if (savedAt25 == False) and (psnr >= 25) and (psnr < 30):
            ...
            saveImage(t, 30)
            savedAt30 = True
            #model_0.hash_table.requires_grad_(False) # turn off grad for hash table
            #print('\nturned off hash gradients\n')
    ...
    plt.xlabel('Epoch')
    plt.savefig("c_elegans_psnr_plots_frozen_hash/hash_nerf_reconstruction_frame_"+str(t*40)+".png", bbox_inches="tight")
    plt.close()
...
#epilogue
np.savetxt('time_series_frozen_hash.csv', time_series, delimiter=',', fmt='%f')
np.savetxt('epoch_series_frozen_hash.csv', epoch_series, delimiter=',', fmt='%f')
np.savetxt('loss_series_frozen_hash.csv', loss_series, delimiter=',', fmt='%f')
np.savetxt('loss_at_epoch_frozen_hash.csv', loss_at_epoch, delimiter=',', fmt='%f')
np.savetxt('psnr_at_epoch_frozen_hash.csv', psnr_at_epoch, delimiter=',', fmt='%f')
#plot net psnrs
plt.plot(range(0,len(psnr_at_epoch)), psnr_at_epoch)
plt.title('Train PSNR per epoch')
plt.ylabel('PSNR')
plt.xlabel('Epoch')
plt.savefig("c_elegans_psnr_plots_frozen_hash/hash_nerf_reconstruction_all_frames.png", bbox_inches="tight")
plt.close()
...
plt.ylabel('MSE Loss')
plt.xlabel('Epoch')
plt.savefig("c_elegans_psnr_plots_frozen_hash/hash_nerf_reconstruction_all_frames_loss.png", bbox_inches="tight")
plt.close()
print("Training Finished")
