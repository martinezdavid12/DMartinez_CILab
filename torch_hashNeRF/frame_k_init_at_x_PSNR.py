...
def saveImage(frame_note, psnr_note=39):
    ...
    with torch.inference_mode():
        ...
        plt.axis(False)
        plt.savefig("c_elegans_reconstructions_psnr_25_prev/hash_nerf_reconstruction_frame_"+str(frame_note)+"_psnr_"+str(psnr_note)+".png", bbox_inches="tight", pad_inches=0.0)
        plt.close()
...
for t in range(0, numFrames):
    ...
    print("working on frame:", workingFrame)
    ...
    for epoch in tqdm(range(0,60)):
        ...
        if (savedAt25 == False) and (psnr >= 25) and (psnr < 30):
            ...
            psnr_25_model_path = saveModel(model_0, 25, t) # saves model at this psnr
            saveImage(t, 25)
            savedAt25 = True
    ...
    plt.xlabel('Epoch')
    plt.savefig("c_elegans_psnr_plots_25_prev/hash_nerf_reconstruction_frame_"+str(t*40)+".png", bbox_inches="tight")
    plt.close()
    print("loading model from psnr 25")
    model_0.load_state_dict(torch.load(psnr_25_model_path))
    model_0.train()
...
#epilogue
np.savetxt('time_series_psnr_25_prev.csv', time_series, delimiter=',', fmt='%f')
np.savetxt('epoch_series_psnr_25_prev.csv', epoch_series, delimiter=',', fmt='%f')
np.savetxt('loss_series_psnr_25_prev.csv', loss_series, delimiter=',', fmt='%f')
np.savetxt('loss_at_epoch_psnr_25_prev.csv', loss_at_epoch, delimiter=',', fmt='%f')
np.savetxt('psnr_at_epoch_psnr_25_prev.csv', psnr_at_epoch, delimiter=',', fmt='%f')
#plot net psnrs
plt.plot(range(0,len(psnr_at_epoch)), psnr_at_epoch)
plt.title('Train PSNR per epoch')
plt.ylabel('PSNR')
plt.xlabel('Epoch')
plt.savefig("c_elegans_psnr_plots_25_prev/hash_nerf_reconstruction_all_frames.png", bbox_inches="tight")
plt.close()
...
plt.ylabel('MSE Loss')
plt.xlabel('Epoch')
plt.savefig("c_elegans_psnr_plots_25_prev/hash_nerf_reconstruction_all_frames_loss.png", bbox_inches="tight")
plt.close()
print("Training Finished")
